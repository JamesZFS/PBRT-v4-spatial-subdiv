...
GuidedPathIntegrator::GuidedPathIntegrator(const int maxDepth, const int minRRDepth, const bool useNEE, const GuidingSettings guideSettings, const RGBColorSpace *colorSpace, Camera camera, Sampler sampler,
    ...
    : RayIntegrator(camera, sampler, aggregate, lights),
      ...
      lightSampler(LightSampler::Create(lightSampleStrategy, lights, Allocator())) {
        ...
        if (guideSettings.treeangulardistancethreshold == 1.0f) guiding_fieldSubdivConfig.angularDistanceThreshold = PGL_SPATIAL_ANGULAR_1_DEG;
        else if (guideSettings.treeangulardistancethreshold == 2.0f) guiding_fieldSubdivConfig.angularDistanceThreshold = PGL_SPATIAL_ANGULAR_2_DEG;
        ...
        else throw std::runtime_error("treeangulardistancethreshold must be one of {1, 2, 3, 10} deg");
...
SampledSpectrum GuidedPathIntegrator::Li(Point2i pPixel, RayDifferential ray, SampledWavelengths &lambda,
    ...
    while (true) {
        ...
        if (settings.regularize && anyNonSpecularBounces) {
            ++regularizedBSDFs;
            if(bsdf.GetRoughness() < 0.01f){
                wasSpecularBounce = true;
            }
            else {
        ...
        if (guideRR && depth > settings.minRRDepth) {
            survivalProb = (specularBounce || wasSpecularBounce) ? 0.95 : openpgl::cpp::util::GuidedRussianRoulette(OPGLVector3f(beta), OPGLVector3f(adjointEstimate), OPGLVector3f(pixelContributionEstimate), 0.1f);
        }
#endif
...
#ifdef GUIDED_RR
            survivalProb = (specularBounce || wasSpecularBounce) ? 0.95 : openpgl::cpp::util::StandardThroughputBasedRussianRoulette(OPGLVector3f(rrThroughputWeight));
#else
            survivalProb = (specularBounce || wasSpecularBounce) ? 0.95 : std::max(0.f, std::min(1.f, rrThroughputWeight.MaxComponentValue()));
#endif
        }
...
void GuidedPathIntegrator::LogFileRow(FILE *logFile) const {
    if (guideSettings.enableGuiding) {
        ...
        fprintf(logFile, "%.3f, %ld, %ld, %ld, %ld, %ld, %ld, %.3f, ", 0.f, 0l, 0l, 0l, 0l, 0l, 0l, prevAvgPathLength);
    }
}
